---
title: "Beyond inductive datatypes exploring Self types"
description: Kind is a proof-gramming language that can be used to define inductive datatypes and prove mathematical theorems. Unlike other proof assistants, Kind doesn't include a complex native induction system. Instead, it extends type-theory with a simple primitive, 'Self', which empowers the core language.
published: true
...
_Edit: after thinking about it, this isn't different from just functions that
...
section if you just want to explore self types._
...
From ncatlab, _"Higher inductive types (HITs) are a generalization of inductive
types which allow the constructors to produce, not just points of the type being
defined, but also elements of its iterated identity types."_ In simpler words,
...
This pair type has an extra constructor, `swap`, that enforces that `(a,b) == (b,a)`. Because of that, whenever we pattern-match on `UnordPair`, we must prove
