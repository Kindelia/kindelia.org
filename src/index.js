...
import LevelDebug from "./pages/enqine/screens/LevelDebug";
import {shape_create, shape_empty} from "./pages/enqine/utils/shape.js";
import "./pages/enqine/EnQIne.css";
...
function level(randoms, draw) {
  return function(init_seed) {
    if (!init_seed) {
      init_seed = Math.floor(Math.random() * (2 ** 32));
    }
    var seed = init_seed;
...
var main_level = levels.sipher_art_0;

export default function Debug() {
  return (
    <main className="enqine-app">
      <LevelDebug levelBuilder={main_level} />
    </main>
  );
}
